...
(
    user_id integer NOT NULL GENERATED BY DEFAULT AS IDENTITY ( INCREMENT 1 START 1 MINVALUE 1 MAXVALUE 100000 CACHE 1 ),
...
(
    ...
    gptkey character varying COLLATE pg_catalog."default",
    CONSTRAINT workspaces_pkey PRIMARY KEY (id),
    CONSTRAINT workspaces_user_id_fkey FOREIGN KEY (user_id)
        REFERENCES metadata.users (user_id) MATCH SIMPLE
        ON UPDATE NO ACTION
        ON DELETE NO ACTION
)""")
    connection.commit()
...
(
    ...
    CONSTRAINT workspace_history_user_id_fkey FOREIGN KEY (user_id)
        REFERENCES metadata.users (user_id) MATCH SIMPLE
        ON UPDATE NO ACTION
        ON DELETE NO ACTION
)
...
def create_workspace(workspace_name,user_id,id=0):
    connection = connect_to_database()
    cursor = connection.cursor()
    ...
    if selected == 'Workspaces':
        ...
        if st.button('Create Workspace', key='create_workspace_button'):
            try:
                ...
                cursor = connection.cursor()
                id=0
                create_workspace(workspace_name, user_id,id)
                ...
                workspace_created, workspace_id ,user_id= create_workspace(workspace_name,user_id,id)
